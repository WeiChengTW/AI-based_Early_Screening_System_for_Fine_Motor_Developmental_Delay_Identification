...
def _user_allowed_uid(user: dict) -> str:
    return str(user.get("target_uid") or user.get("account") or "").strip()


def _can_access_uid(uid: str) -> bool:
    user = _current_user()
    level = _user_level(user)
    if level <= 0:
        return False
    if level == 1:
        return uid == _user_allowed_uid(user)
    return True


def _extract_uid_from_kid_path(raw_path: str) -> str:
    p = (raw_path or "").replace("\\", "/").lstrip("/")
    if p.startswith("kid/"):
        p = p[4:]
    if "/" not in p:
        return ""
...
def _extract_uid_from_image_path(raw_path: str) -> str:
    p = (raw_path or "").replace("\\", "/").strip()
    if not p:
        return ""

    # 不允許外部完整 URL，避免繞過本機權限控管。
    if p.startswith("http://") or p.startswith("https://"):
        return ""

    p = p.lstrip("/")
    for prefix in ("kid/", "images/"):
...
def kid(relpath):
    ...
    req_uid = relpath.split("/", 1)[0]
    if not _can_access_uid(req_uid):
        return ("", 403)
...
def view_compare():
    ...
    if not _can_access_uid(uid):
        return "Forbidden", 403

    if img_path:
        path_uid = _extract_uid_from_image_path(img_path)
        if not path_uid:
            return "Forbidden", 403
        if path_uid != uid:
            return "Forbidden", 403
        if not _can_access_uid(path_uid):
            return "Forbidden", 403

        # 僅允許站內相對路徑，杜絕前端直接塞外部連結。
    ...
    try:
        ...
        for r in rows:
            ...
            if real_path:
                ...
                r["compare_url"] = (
                    f"/view-compare?uid={uid}&task_id={tid}&img=/{real_path}"
                )
            else:
